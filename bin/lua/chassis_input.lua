...
-- Generate chassis and input values
...
--Get input values from user, use form chassis.html
local values = NB:InputValue("html/chassis.html")

--Check that input values are ok
local ok = true
if tonumber(values["segments"]) == nil then
	ok = false
elseif tonumber(values["width"]) == nil then
	ok = false
...
if ok then
	...
	segments = tonumber(values["segments"])
	NB:Log("Values ok")
	generate()
else
	NB:Log("Values are not numbers")
end